...
template <typename T>
struct HasTaskTypeNameField<T,
                            std::void_t<decltype(std::declval<T>().task_type_name)>>
    : std::true_type {};
...
template <typename T>
struct HasDistanceToleranceField<
    T, std::void_t<decltype(std::declval<T>().distance_tolerance)>> : std::true_type {};
...
template <typename T>
struct HasHeadingToleranceField<
    T, std::void_t<decltype(std::declval<T>().heading_tolerance)>> : std::true_type {};
...
struct L2Ros1Recorder::Impl {
    ...
    void RefreshSampleMode() {
        ...
        l2_log::SetSampleMode(ParseSampleMode(sample_mode));
        ROS_INFO_STREAM("L2 recorder sample_mode updated to " << sample_mode);
    }

    void StartAgent() {
        agent = std::make_unique<LogAgent>(root_dir);
        const auto result = agent->Start();
        agent_started = result.success;
        if (!result.success) {
            ROS_WARN_STREAM("failed to start log agent: " << result.message);
        }
    }

    void StopAgent(bool drain) {
        if (!agent) {
            return;
        }
        if (!agent_started) {
        ...
        const auto result = agent->Stop(drain);
        if (!result.success) {
            ROS_WARN_STREAM("failed to stop log agent: " << result.message);
        }
        agent_started = false;
    }
    ...
    void DrainAgent() {
        ...
        const auto result = agent->DrainNow();
        if (!result.success) {
            ROS_WARN_STREAM("failed to drain log agent: " << result.message);
        }
    }
    ...
    void RecordSerialized(const std::string& topic,
                          const std::string& payload,
                          const L2RosRecordMetadata& metadata) {
        l2_log::RecordTopicWithBusinessSemantics(
            ToL2TopicMessage(BuildRosTopicMessage(topic, payload, metadata)));
    }
    ...
    void OnOdometry(const OdomInfoMsg::ConstPtr& msg) {
        ...
        RecordSerialized(kOdomInfoTopic, SerializeMessage(*msg), metadata);
    }
    ...
    void OnMap(const MapMsg::ConstPtr& msg) {
        ...
        RecordSerialized(kMapTopic, SerializeMessage(*msg), metadata);
    }
    ...
    void OnLocalMap(const LocalMapMsg::ConstPtr& msg) {
        ...
        RecordSerialized(kLocalMapTopic, SerializeMessage(*msg), metadata);
    }
    ...
    void OnLocationCode(const LocationCodeMsg::ConstPtr& msg) {
        ...
        RecordSerialized(kLocationCodeTopic, SerializeMessage(*msg), metadata);
    }
    ...
    void OnPerceptionCode(const PerceptionCodeMsg::ConstPtr& msg) {
        ...
        RecordSerialized(kPerceptionCodeTopic, SerializeMessage(*msg), metadata);
    }
    ...
    void OnNavigationCode(const NavigationCodeMsg::ConstPtr& msg) {
        ...
        metadata.payload_summary = {
            ...
            {"primary_fault_code",
             msg->faults.empty() ? "0"
                                 : std::to_string(msg->faults.front().code)},
            {"primary_fault_msg", msg->faults.empty() ? "" : msg->faults.front().msg},
        };
        RecordSerialized(kNavigationCodeTopic, SerializeMessage(*msg), metadata);
    }
    ...
    void OnCmdVel(const CmdVelMsg::ConstPtr& msg) {
        ...
        RecordSerialized(kCmdVelCalibTopic, SerializeMessage(*msg), metadata);
    }
    ...
    void OnChassisAgvState(const ChassisAgvStateMsg::ConstPtr& msg) {
        ...
        RecordSerialized(kChassisAgvStateTopic, SerializeMessage(*msg), metadata);
    }
    ...
    void OnNavigationGoal(const NavigationGoalMsg::ConstPtr& msg) {
        ...
        summary.header_stamp_us = summary.header_stamp_ns / 1000;
        ...
        auto metadata =
            BuildNavigationGoalMetadata(current_task_id, summary, BuildMetadata(*msg));
        RecordSerialized(kNavigationGoalTopic, SerializeMessage(*msg), metadata);
    }
    ...
    void OnNavigationFeedback(const NavigationFeedbackMsg::ConstPtr& msg) {
        ...
        auto metadata =
            BuildNavigationFeedbackMetadata(task_id, summary, BuildMetadata(*msg));
        RecordSerialized(kNavigationFeedbackTopic, SerializeMessage(*msg), metadata);
    }
    ...
    void OnNavigationResult(const NavigationResultMsg::ConstPtr& msg) {
        ...
        auto metadata =
            BuildNavigationResultMetadata(task_id, summary, BuildMetadata(*msg));
        RecordSerialized(kNavigationResultTopic, SerializeMessage(*msg), metadata);
...
int L2Ros1Recorder::Run(int argc, char** argv, const std::string& node_name) {
    ...
    std::signal(SIGINT, HandleRecorderSignal);
    std::signal(SIGTERM, HandleRecorderSignal);
    ros::NodeHandle nh;
    ros::NodeHandle pnh("~");
