...
int main(int argc, char** argv) {
    ...
    naviai::log::test::ScenarioConfig next;
    next.root_dir = root_dir;
    next.business_messages = 4;
    naviai::log::test::RunScenario(next);
    const auto inspection = naviai::log::test::InspectRoot(root_dir);
    const bool passed = recovered && inspection.counts.active_files == 0 &&
                        inspection.idx_records > 0;
    return naviai::log::test::ReportResult(
        kCaseId, passed, "recovered recorder continued writing", inspection);
}
