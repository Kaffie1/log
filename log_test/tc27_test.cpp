#include "tc_support.hpp"

int main(int argc, char** argv) {
    constexpr int kCaseId = 27;
    auto root_dir = argc > 1 ? std::filesystem::path(argv[1])
                             : naviai::log::test::DefaultRootForCase(kCaseId);
    naviai::log::test::ScenarioConfig config;
    config.root_dir = root_dir;
    config.segment_size_bytes = 32 * 1024;
    config.payload_size = 1024;
    config.business_messages = 300;
    config.static_messages = 10;
    config.large_messages = 10;
    config.enable_agent = true;
    config.post_record_sleep_ms = 500;
    const auto run = naviai::log::test::RunScenario(config);
    const bool passed =
        run.inspection.sequences.size() == 320 && run.inspection.counts.active_files == 0;
    return naviai::log::test::ReportResult(
        kCaseId, passed, "controlled long-run sample verified", run.inspection);
}
