...
int main(int argc, char** argv) {
    ...
    const bool passed = run.inspection.counts.gz_idx > 0 &&
                        run.inspection.idx_records >= run.inspection.sequences.size();
    return naviai::log::test::ReportResult(
        kCaseId, passed, "compressed index remained readable", run.inspection);
}
