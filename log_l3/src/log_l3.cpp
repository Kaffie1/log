...
std::string BuildStructuredPayload(const L3SdkOptions& options,
    ...
    const auto event_time_us = timestamp_us > 0 ? timestamp_us : NowUs();
    const auto receive_time_us = NowUs();

    std::string json = fmt::format(
        "{{\"version\":\"1.0\",\"timestamp_us\":{},\"receive_timestamp_us\":{},"
        "\"robot_sn\":\"{}\",\"layer\":\"L3\",\"module\":\"{}\",\"level\":\"{}\","
        "\"payload\":\"{}\"",
        event_time_us,
        receive_time_us,
        EscapeJson(options.robot_sn),
...
LoggerConfig BuildLoggerConfig(const L3SdkOptions& options,
                              const std::filesystem::path& active_path) {
    LoggerConfig config;
    config.root_dir = active_path.parent_path().string();
    config.file_name = active_path.filename().string();
    config.level = options.level;
    // L3 emits a fully structured JSON line itself, so writer should pass it
    // through as plain text instead of wrapping it again.
...
void EnsureInitialized() {
    if (!State().initialized) {
        throw std::runtime_error("L3 is not initialized");
    }
}

LogService BuildLogService(const L3SdkOptions& options) {
    return LogService(std::filesystem::path(options.root_dir));
}

std::filesystem::path BuildActiveLogPath(const L3SdkOptions& options) {
    const auto plan = BuildLogService(options).BuildActiveFilePlan("", options.file_name);
    if (!plan.has_value()) {
        throw std::runtime_error("failed to build L3 active log plan");
    }
    return plan->path;
}

void RecoverActivePathIfNeeded(const L3SdkOptions& options,
                               const std::filesystem::path& active_path,
...
void RecoverLingeringActiveFiles(const L3SdkOptions& options) {
    ...
    if (!task.success) {
        throw std::runtime_error("failed to recover lingering active files: " + task.message);
    }
}

void RunDrainPass(const L3SdkOptions& options) {
    LogAgent agent(std::filesystem::path(options.root_dir), options.agent_policy);
    auto result = agent.DrainNow();
    if (!result.success) {
        throw std::runtime_error("failed to drain L3 files: " + result.message);
    }
}
...
void L3::Init(const L3SdkOptions& options) {
    auto& state = State();
    ...
    if (options.enable_runtime_agent) {
        ...
        if (!result.success) {
            state.agent.reset();
            throw std::runtime_error("failed to start L3 agent: " + result.message);
        }
    } else {
        state.agent.reset();
    }
...
void L3::SetLevel(LogLevel level) {
    LogManager::SetLevel(level);
}

void L3::SetLevel(LoggerModule module, LogLevel level) {
...
void L3::Write(LoggerModule module,
               LogLevel level,
               std::string_view payload,
               const char* file,
               int line,
...
void L3::Write(const L3LogInput& input,
               const char* file,
               int line,
               const char* func) {
    Write(input.module,
