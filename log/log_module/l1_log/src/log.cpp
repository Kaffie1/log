...
ModuleMetadata BuildL1ModuleMetadata(std::string_view module_name,
                                     const ModuleRegistrationOptions& options) {
    ModuleMetadata metadata;
    metadata.module_name = std::string(module_name);
    metadata.layer_metadata.layer = LogLayer::L1;
    metadata.layer_metadata.category =
        options.category.empty() ? "system" : options.category;
...
void L1::RegisterModule(const std::string& module_name,
                        const ModuleOptions& options) {
    LogApi::RegisterModule(
        BuildL1ModuleMetadata(module_name, ToInternalOptions(options)));
}
...
void L1::SetLevelForAll(LogLevel level) {
