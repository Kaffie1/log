...
ModuleMetadata BuildL2ModuleMetadata(std::string_view module_name,
                                     const ModuleRegistrationOptions& options) {
    ModuleMetadata metadata;
    metadata.module_name = std::string(module_name);
    metadata.layer_metadata.layer = LogLayer::L2;
    metadata.layer_metadata.category =
        options.category.empty() ? "replay" : options.category;
    metadata.layer_metadata.subtype =
        options.subtype.empty() ? "record" : options.subtype;
    metadata.layer_metadata.event_type =
        options.event_type.empty() ? "record" : options.event_type;
...
void InitLogLayer(LogLevel level, const std::string& root_dir) {
    ...
    config.sink.json_output = true;
    config.sink.enable_console = false;
    LogApi::Init(config);
}

void RegisterModule(const std::string& module_name, const ModuleOptions& options) {
    LogApi::RegisterModule(
        BuildL2ModuleMetadata(module_name, ToInternalOptions(options)));
}
...
void InitRecorder(const L2RecorderOptions& options) {
    ...
    for (const auto& descriptor : options.topics) {
        if (descriptor.source_domain == L2SourceDomain::Sensor) {
            RegisterSensorTopic(descriptor);
            continue;
        }
