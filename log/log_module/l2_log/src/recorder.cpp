...
std::string DomainName(L2SourceDomain domain) {
    return domain == L2SourceDomain::Sensor ? "sensor" : "algorithm";
}
...
std::string BuildModuleName(const L2TopicDescriptor& descriptor) {
    if (descriptor.source_domain == L2SourceDomain::Sensor) {
        if (!descriptor.source_type.empty()) {
            return Sanitize(descriptor.source_type);
        }
        return Sanitize(descriptor.source_module);
    }
